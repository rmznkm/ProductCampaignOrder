...
{
    public class CreateProductStringCommandHandlerItemTests : TestsFor<CreateProductStringCommandHandlerItem>
    {
        [Fact]
        public void CanHandle_Ifcreate_product_ReturnsTrue()
        {
            var result = Instance.CanHandle("create_product");
            result.Should().BeTrue();
        }

        [Fact]
        public void CanHandle_IfNotcreate_product_ReturnsFalse()
        {
        ...
        {
            var commands = new List<string> { "P1", "0", "0" };
            await Instance.HandleAsync(commands);

            GetMockFor<ICommandSender>().Verify(x => x.SendAsync(It.IsAny<CreateProductCommandRequest>()));
        }
    }
}
